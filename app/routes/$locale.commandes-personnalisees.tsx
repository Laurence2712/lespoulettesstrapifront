...
export default function CommandesPersonnalisees() {
  ...
  const STEPS = [
    { num: '01', title: t('custom.step1_title_short'), desc: t('custom.step1_desc_detail'), emoji: '💬' },
    { num: '02', title: t('custom.step2_title_short'), desc: t('custom.step2_desc_detail'), emoji: '✏️' },
    { num: '03', title: t('custom.step3_title_short'), desc: t('custom.step3_desc_detail'), emoji: '🧵' },
    { num: '04', title: t('custom.step4_title_short'), desc: t('custom.step4_desc_detail'), emoji: '📦' },
  ];
        ...
        <div className="px-6 sm:px-10 md:px-16 lg:px-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
            <div>
              ...
              <p className="anim-fade-up font-basecoat text-sm sm:text-base text-gray-700 dark:text-gray-300 leading-relaxed mb-8" data-delay="0.2">
        ...
        <div className="px-6 sm:px-10 md:px-16 lg:px-24">
          ...
          <div className="space-y-0 divide-y divide-gray-100 dark:divide-gray-800">
            {STEPS.map((step, i) => (
              <div
                key={step.num}
                className={`anim-fade-up grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center py-10 sm:py-14 ${i % 2 === 1 ? 'md:flex md:flex-row-reverse' : ''}`}
                data-delay={`${i * 0.1}`}
              >
                {/* Grand numéro */}
                <div className={`flex items-center gap-6 ${i % 2 === 1 ? 'md:justify-end' : ''}`}>
                  <span className="font-basecoat text-[6rem] sm:text-[8rem] font-bold leading-none text-benin-jaune/20 select-none flex-shrink-0">
                    {step.num}
                  </span>
                ...
                {/* Texte */}
                <div>
                  <h3 className="font-basecoat text-base sm:text-lg font-bold uppercase text-gray-900 dark:text-gray-100 mb-3 tracking-wide">
                    {step.title}
                  </h3>
                  <p className="font-basecoat text-sm sm:text-base text-gray-600 dark:text-gray-400 leading-relaxed">
                    {step.desc}
                  </p>
                </div>
              </div>
      ...
      <section className="bg-beige dark:bg-gray-950 px-6 sm:px-10 md:px-16 lg:px-24 py-14 sm:py-18 md:py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start">
          <div className="anim-fade-right" data-delay="0.1">
            ...
            <ul className="space-y-4 font-basecoat text-sm sm:text-base text-gray-700 dark:text-gray-300">
              {CREATIONS.map((item) => (
                <li key={item} className="flex items-start gap-3 border-b border-gray-200 dark:border-gray-700 pb-4 last:border-0 last:pb-0">
                  <span className="text-benin-jaune font-bold flex-shrink-0 mt-0.5">◆</span>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
            <p className="font-basecoat text-sm text-gray-400 dark:text-gray-500 mt-6 italic">
              {t('custom.more_ideas')}
            </p>
          </div>

          <div className="anim-fade-left" data-delay="0.2">
            <h3 className="font-basecoat text-base font-bold uppercase text-gray-900 dark:text-gray-100 mb-6 tracking-wide">
              ...
              {INFOS.map((info, index) => (
                <div
                  ...
                  className={`bg-white dark:bg-gray-900 rounded-lg overflow-hidden border transition-all duration-300 ${
                    openIndex === index ? 'border-benin-jaune/40 shadow-md' : 'border-gray-100 dark:border-gray-700 hover:border-benin-jaune/25'
                  }`}
                >
                  <button
                  ...
                  >
                    <span className={`font-bold text-sm uppercase transition-colors duration-200 ${openIndex === index ? 'text-benin-jaune' : 'text-gray-900 dark:text-gray-100'}`}>
                      {info.title}
                    </span>
                    <svg
                      className={`w-5 h-5 flex-shrink-0 transition-transform duration-300 ${openIndex === index ? 'rotate-180 text-benin-jaune' : 'text-gray-300'}`}
                  ...
                  <div className={`overflow-hidden transition-all duration-500 ${openIndex === index ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="px-4 pb-4 pt-1 font-basecoat text-sm text-gray-600 dark:text-gray-300 leading-relaxed border-t border-gray-100 dark:border-gray-700">
                      {info.content}
                    </div>
                  </div>
                </div>
