...
export default function Index() {
  ...
  useEffect(() => {
    async function fetchHomepageData() {
      try {
        const response = await fetch(getApiUrl('/api/homepages?populate=*'));
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
        if (data?.data?.length) {
          const homepage = data.data[0];
          const bannerImageUrl = homepage.banner_image?.formats?.large?.url
            ? `${STRAPI_URL}${homepage.banner_image.formats.large.url}`
            : homepage.banner_image?.url
            ? `${STRAPI_URL}${homepage.banner_image.url}`
            : '';
          let descriptionText = '';
      ...
      try {
        ...
        if (data?.data) {
          const realisationsData: Realisation[] = data.data.map((item: any) => ({
            ...
            image_url: item.Images?.[0]?.url ? `${STRAPI_URL}${item.Images[0].url}` : undefined,
    ...
    async function fetchActualites() {
      try {
        const response = await fetch(
          getApiUrl('/api/actualites?populate=*&sort[0]=publishedAt:desc&pagination[limit]=1')
        );
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
        if (data?.data) {
          const actualitesData: Actualite[] = data.data.map((item: any) => ({
            ...
            image_url: item.image?.formats?.large?.url
              ? `${STRAPI_URL}${item.image.formats.large.url}`
              : item.image?.url
