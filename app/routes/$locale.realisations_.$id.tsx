...
export default function RealisationDetail() {
  ...
  const handleShare = async () => {
    ...
    const shareData = { title: realisation.title, text: `Découvre ${realisation.title} sur Les Poulettes`, url };
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 lg:gap-16">
          ...
          <div className="anim-fade-left flex flex-col font-basecoat">
            ...
            <div className="anim-expand-line w-24 sm:w-28 h-[2px] bg-gradient-to-r from-benin-jaune via-benin-jaune/60 to-transparent mt-3 mb-4"></div>

            {/* Partage */}
            <div className="flex items-center gap-2 mb-5">
              <button
                onClick={handleShare}
                className="inline-flex items-center gap-1.5 font-basecoat text-xs text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 border border-gray-200 dark:border-gray-700 rounded-full px-3 py-1.5 transition hover:border-gray-400"
              >
                {shareCopied ? (
                  <><svg className="w-3.5 h-3.5 text-benin-vert" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg> Lien copié !</>
                ) : (
                  <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 103.935 2.186 2.25 2.25 0 00-3.935-2.186zm0-12.814a2.25 2.25 0 103.933-2.185 2.25 2.25 0 00-3.933 2.185z" /></svg> Partager</>
                )}
              </button>
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`${realisation.title} — Les Poulettes : ${typeof window !== 'undefined' ? window.location.href : ''}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 font-basecoat text-xs text-gray-500 dark:text-gray-400 hover:text-green-600 border border-gray-200 dark:border-gray-700 rounded-full px-3 py-1.5 transition hover:border-green-400"
              >
            ...
            {realisation.description?.trim() && (
              <div className="mb-6 p-5 bg-white dark:bg-gray-900 border-l-4 border-benin-jaune rounded-r-xl">
