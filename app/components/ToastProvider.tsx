...
const ICONS: Record<ToastType, React.ReactNode> = {
  success: (
    <span className="w-7 h-7 rounded-full bg-benin-jaune flex items-center justify-center flex-shrink-0">
      <svg className="w-3.5 h-3.5 text-black dark:text-gray-100" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
      </svg>
    </span>
  ),
  error: (
    <span className="w-7 h-7 rounded-full bg-white dark:bg-gray-900/20 flex items-center justify-center flex-shrink-0">
...
const PROGRESS_COLORS: Record<ToastType, string> = {
  ...
  error:   'bg-white dark:bg-gray-900/60',
  info:    'bg-white dark:bg-gray-900/60',
};
      ...
      >
        {toasts.map(toast => (
          ...
          >
            ...
            {/* Progress bar */}
            <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-white dark:bg-gray-900/10 rounded-b-2xl overflow-hidden">
              <div
