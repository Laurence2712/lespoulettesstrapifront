...
export default function NewsletterForm({ variant = 'dark' }: NewsletterFormProps) {
  ...
  if (isSuccess) {
    return (
      <div className={`flex items-center gap-3 font-basecoat text-sm sm:text-base ${isLight ? 'text-benin-vert' : 'text-benin-vert'}`}>
        <span className="w-7 h-7 rounded-full bg-benin-vert/100 flex items-center justify-center flex-shrink-0">
  ...
  return (
    ...
    >
      ...
      {fetcher.data?.error && (
        <p className={`text-xs mt-1 font-basecoat ${isLight ? 'text-benin-rouge' : 'text-benin-rouge'}`}>
          {fetcher.data.error}
        </p>
      )}
    </form>
